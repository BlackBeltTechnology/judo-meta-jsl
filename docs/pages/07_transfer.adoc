...
Use the `field` keyword to specify a field within a transfer object. The full list of modifiers can be found in the xref:appx_syntax.adoc#transfer-field[Appendix].
...
Use the `relation` keyword to specify a relation between two transfer objects. The full list of modifiers can be found in the xref:appx_syntax.adoc#transfer-relation[Appendix].
...
A mapped field is a simple field with a mapping configuration. The full list of modifiers can be found in the xref:appx_syntax.adoc#transfer-field[Appendix].
...
[purple]#**field**# &lt;primitive> &lt;name> [purple]##**<=**## &lt;expression>
    ...
    &#x5B;[purple]##**default:**##&lt;default>]
    &#x5B;[purple]##**bind:**##([purple]##**true**##|[purple]##**false**##)][purple]#**;**#
----
...
If the `bind` modifier is `true`, changes in the transfer field value will set the value of the mapped entity's field. It's important to note that the `bind` modifier only works for expressions that select a non-calculated field.
...
transfer SalesPersonTransfer(SalesPerson salesperson) {
    field String firstName <= salesperson.firstName bind enabled;
    field String lastName <= salesperson.lastName bind enabled;
}
----

In other words, the `\<=` symbol creates a read-only connection between the entity and the transfer fields, where the value of the transfer field takes the value of the expression. However, if the `bind` modifier is `true`, it defines a read-write connection.

=== Mapped relation

So far, expressions returned primitive values. The following example shows a relation that is mapped to one of the entity's relations. The full list of modifiers can be found in the xref:appx_syntax.adoc#transfer-relation[Appendix].
...
[IMPORTANT]
====
Only static actions can be defined in unmapped transfer objects.
====
