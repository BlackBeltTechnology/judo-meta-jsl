...
Initial -right-> Persisted : create
...
* The *build* event represents the action of initializing an object, causing it to transition to a defined state. It sets the initial field values of the object.

* The *create* event signifies the action of saving an object's state to the storage. It transitions the object from an initial or modified state to a persisted state.

* The *fetch* event indicates the retrieval of data from the persistent storage. It transitions the object into a state where it holds the fetched data.

* The *set* event represents an action where the mapped transfer object's fields modified. This can lead to a transition from a persisted state to a modified state, indicating that changes have been made but not yet saved.

* The *update* event is associated with saving the modifications made to a transfer object. It causes the object to transition from a modified state back to a persisted state, reflecting that the changes have been persisted.
