...
Although the initialize event handler is where transfer fields and relations can be set, the field's default value is always set before the initializer is called.

An initialize event handler can be defined both for unmapped and mapped transfer objects.

[NOTE]
====
Currently, the body of the initialize event handler can only be defined in Java.
====
...
[subs="quotes"]
----
[purple]#**event initialize**# &#x5B;[purple]##**()**##][purple]##**;**##
----
...
The create event handler can optionally accept a transfer object parameter. The field values of this parameter can be utilized when the transfer object is saved for the first time. This parameter enables differentiation between the creation and update parameters of the transfer object.
