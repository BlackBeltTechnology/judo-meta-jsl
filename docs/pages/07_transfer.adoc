...
Optionally, a <default> value can be specified as an expression. The <default> value expression is evaluated when a new instance of the transfer object is created, and the field is set according to the default value expression.
...
transfer PersonTransfer(Person person) {
	field String fullName = person.firstName + " " + person.lastName ;
}
----
...
transfer SalesPersonTransfer(SalesPerson salesperson) {
    field String name = salesperson.firstName + " " + salesPerson.lastName ;
...
// Service and export description
