...
JSL has a set of words that are reserved words and cannot be used directly as element names. If you still want to use these reserved words as element names, the words must be enclosed in back-tick (```) characters as shown in the example below.

----
field String `model`
----

This is the list of reserved words in the JSL.
...
[subs="quotes"]
----
[purple]#**import**# &lt;model> &#x5B;[purple]##**as**## &lt;alias>]
----
...
type string String(max-length = 4000)
----
...
Data model is the core part of the domain model that captures the real-world entities and rules that determine how data can be created, changed, and deleted.

The persistent data of a domain model is described in the form of entity types. In other words, an entity type is a definition of a set of similar instances.

Actual data is stored by creating instances of entity types. An entity type (or shortly an entity) may have identifiers, stored fields, derived fields, relations and constraints that are referred to as members of an entity. See section <<Members>>.
...
An entity can contain data descriptions called fields. A primitive field has a domain primitive type, in other words, it is a primitive typed element.

Primitive fields cannot store multiple primitive values, but only a single primitive value. To put it another way, there is no list of primitives.
...
[subs="quotes"]
----
[purple]#**field**# &lt;primitive> &lt;name> &#x5B;[purple]##**=**## &lt;default>]
----
...
where the <name> is the name of the domain model binary, the <mime-types> is a list of media types indicating the accepted formats, and the <max-file-size> is the maximum size of the file.
...
The <mime-types> is a comma-separated list of strings representing media types. A media type string must consist of a type and a subtype separated by `/` in the following format:
...
Valid media types and subtypes can only contain alphabetic characters, digits and `.`, `_`, `-`, `+`.
...
|Returns a string where all occurrences of <oldstring> in the original string replaced by <newstring>.
...
|Adds duration expressed in <milliseconds> to the original time. <milliseconds> must be an integer. If <milliseconds> is less than 0 it subtracts duration from the original time.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**plusSeconds(seconds = **#<seconds>[purple]##**)**## : timestamp`
|Adds duration expressed in <seconds> to the original time. <seconds> must be an integer. If <seconds> is less than 0 it subtracts duration from the original time.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**plusMinutes(minutes = **#<minutes>[purple]##**)**## : timestamp`
|Adds duration expressed in <minutes> to the original time. <minutes> must be an integer. If <minutes> is less than 0 it subtracts duration from the original time.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**plusHours(hours = **#<hours>[purple]##**)**## : timestamp`
|Adds duration expressed in <hours> to the original time. <hours> must be an integer. If <hours> is less than 0 it subtracts duration from the original time.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**plusDays(days = **#<days>[purple]##**)**## : timestamp`
|Adds duration expressed in <days> to the original time. <days> must be an integer. If <days> is less than 0 it subtracts duration from the original time.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**plusMonths(months = **#<months>[purple]##**)**## : timestamp`
|Adds duration expressed in <months> to the original time. <months> must be an integer. If the result date is invalid, the function sets the day to the last valid day of the month. If the If <months> is less than 0 it subtracts duration from the original time.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**plusYears(years = **#<years>[purple]##**)**## : timestamp`
|Adds duration expressed in <years> to the original time. <years> must be an integer. If <years> is less than 0 it subtracts duration from the original time.
|======================================================================
...
|Example: `self.field!asType(MyApplication::MyEntity)`
|======================================================================
