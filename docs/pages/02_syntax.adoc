...
entity Person {
    field String firstName default:"" required;
    field String lastName default:"" required;;;;;
...
Most JSL statements may include modifiers, which are used to alter the behavior of the statement and convey additional information about them. Typically, modifiers help define the scope, behavior, and restrictions associated with various statements in JSL code, contributing to the overall structure and functionality of the code.

A modifier always begins with a keyword, followed by an optional colon (`:`) and a value. This value can be a constant, an expression, or a reference to another JSL statement, depending on the modifier.

This is a typical JSL statement with two modifiers.

[source,jsl]
----
field String firstName required:true default:"";
----

The `default` modifier sets the initial value of the field to a zero-length string, and the `required` modifier specifies that a value must be provided for the field, making it mandatory.

Modifiers that can take a constant `true` value may omit their value. For example, the `required` modifier may stand with no colon and value, which is identical to `required:true`.

[source,jsl]
----
field String firstName required default:"";
----

The order of modifiers within a single statement has no effect.
