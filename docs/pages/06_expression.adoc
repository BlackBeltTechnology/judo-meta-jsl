...
Constant values, entity members, operators, query executions and function calls can be combined into sequences known as expressions. Among other things, expressions are used to navigate and query the entity model.

An expression is evaluated to a value, which can be primitive value (e.g. `42` or `"apple"`), entity instance or a collection of entity instances.

*Data expressions* return values of primitive types, such as strings or integers. A data expression can be
...
*Reference expressions* return an entity instance or collection of entity instances.
...
// * assignment (`=`,`+=`,`-=`)
* navigation, field selection, query and function call (`.`)
* assignment (`=`)
* arithmetic (`+`, `-`, `*`, `/`, `div`, `mod`, `^`)
* relational (`==`,`!=`,`<`,`>`,`>=`,`\<=`)
* conditional (`? :`) and
...
* a static single reference using the syntax `shop::Order.any()`, which means one arbitrary instance of the Order entity, or
* a query execution using the syntax `shop::MyQuery()`, which means all entity instances returned by the query `shop::MyQuery`
...
relation Products[] orderedProducts <= shop::Order.all().orderItems.product;

// a navigation starting from self variable in Order entity
relation Products[] orderedProducts <= self.orderItems.product;
----
...
If the calculated relation has parameters, pass the arguments of the relation within parentheses.
...
entity abstract Customer {
	relation Order[] ordersSelectByStatus(OrderStatus status)
	    <= self.orders.filter(o | o.status == status);

	relation Product[] deliveredProducts
	    <= self.ordersSelectByStatus(status = OrderStatus#DELIVERED).orderItems.product;
}
----
...
Both calculated and stored field values of an entity can be read in expressions. To read a field value use the `.` (dot) character. The field selection can be chained for navigation expression. The field selection can only be chained for navigation that returns a single instance.
...
If the calculated field has parameters, pass the arguments of the field within parentheses.
...
----
entity abstract Customer {
	field Integer sumByStatus(OrderStatus status)
	    <= self.orders.filter(o | o.status == status).sum(o | o.price);

	field Integer sumOrdered
	    <= self.sumByStatus(status = OrderStatus#ORDERED);

	field Integer sumDelivered
	    <= self.sumByStatus(status = OrderStatus#DELIVERED);
}
----
...
To execute a static query use its name as a navigation starting point and append the arguments in parentheses. If you do not pass arguments to a query append parentheses with no arguments between them.

*Example:*

[source,jsl]
----
relation Customer[] havingOrdersBetween0and10 <= shop::ordersBetween(min = 0, max = 10).customer;
----
...
[source,jsl]
----
field Integer count <= self.orderItems.size();
field Integer nameLength <= self.fullName.size();
----
...
[source,jsl]
----
field Integer averagePrice <= self.orders.avg(order | order.price);
----
...
entity Customer {
    ...
    field Price avgPrice <= self.orders.avg(field = price);
    relation Order[] belowAvgOrders <= self.orders.filter(order | order.price < order.customer.avgPrice);
}
----
...
|`REQUEST`    |_any_
|Get request variable from HTTP Header or URL Parameter

|`ACTOR`|_claim name_
|Returns a claim of current actor.

Get the username of the current actor:
String.getVariable(category = "ACTOR", key = "username")

|`PRINCIPAL`|_access token attribute_
|Returns the attribute value from the access token.

Get username from OAuth 2.0 token:
String.getVariable(category = "PRINCIPAL", key = "preferred_username")
...
The percent sign represents zero, one, or multiple numbers or characters. The underscore represents a single number or character. These symbols can be used in combinations.
|Examples:
|`"apple".ilike(pattern = "%PL_")` is `true`
|======================================================================
...
[#binary_functions,judo-req="false"]
=== Binary functions
...
|Example:
|`OrderStatus#OPEN.asString()` is `"OPEN"`
|======================================================================
...
[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**typeOf(type = **#<entityType>[purple]##**)**## : boolean`
...
|Example:
|`self.customer.typeOf(type = shop::Customer)` is `true` if `self.customer` is a customer, but neither an enterprise nor a person. For the model, see the xref:01_intro.adoc[Introduction] chapter.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**kindOf(type = **#<entityType>[purple]##**)**## : boolean`
...
|Example:
|`self.customer.kindOf(type = shop::Customer)` is `true` if `self.customer` is either a customer, an enterprise, or a person. For the model, see the xref:01_intro.adoc[Introduction] chapter.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**container(type = **#<entityType>[purple]##**)**## : instance`
...
|Example:
|`self.container(type = shop::Customer)` navigates from the Address to the Customer if the container is an instance of Customer or any subtype of Customer (Enterprise or Person). For the model, see the xref:01_intro.adoc[Introduction] chapter.
|======================================================================

[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**asType(type = **#<entityType>[purple]##**)**## : instance`
...
|Example:
|`self.customer.asType(type = shop::Enterprise)` navigates from Order to Customer and it returns an Enterprise instance. If the result of the navigation is a Person, it returns undefined. For the model, see the xref:01_intro.adoc[Introduction] chapter.
|======================================================================
...
|Example:
|`self.memberOf(instances = shop::Enterprise.orders.orderItems.product)` within a product, it checks whether it has ever been ordered by any enterprise. For the model, see the xref:01_intro.adoc[Introduction] chapter.
|======================================================================
...
|Sorts the given collection in ascending order according to the field selected by <expression>, then returns the collection containing the first elements. If there is more than one element in the first place, all elements in the first place are included in the result. The <expression> must navigate to a primitive type field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
| Example:
...
|Sorts the given collection in ascending order according to the field selected by <expression>, then returns the collection containing the last elements. If there is more than one element in the last place, all elements in the last place are included in the result. The <expression> must navigate to a primitive type field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
|Example:
...
|Sorts the given collection in ascending order according to the field selected by <expression>, then returns the collection containing all of the non-last elements. If there is more than one element in the last place, all elements in the last place are omitted from the result. The <expression> must navigate to a primitive type field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
| Example:
...
|Sorts the given collection in ascending order according to the field selected by <expression>, then returns the collection containing all of the non-first elements. If there is more than one element in the first place, all elements in the first place are omitted from the result. The <expression> must navigate to a primitive type field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
|Example:
...
[options="header",grid="none",cols="1a"]
|======================================================================
|`[purple]#**asCollection(type = **#<entityType>[purple]##**)**## : collection`
...
|Example:
|`shop::Order.all().customer.asCollection(type = shop::Enterprise)` returns all enterprises that have at least one order.
|======================================================================
...
|Returns the minimum value from the set of values selected by the <expression>. The <expression> must navigate to a numeric field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter. 
|Example:
...
|Returns the maximum value from the set of values selected by the <expression>. The <expression> must navigate to a numeric field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
|Example:
...
|Returns the average of the set of values selected by the <expression>. The <expression> must navigate to a numeric field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
|Example:
...
|Returns the sum of the set of values selected by the <expression>. The <expression> must navigate to a numeric field. Note that currently <expression> can only select the immediate field of the iterator variable, multi-step navigation is not allowed. More information about iterators in the <<Iterator functions>> chapter.
|Example:
