...
The access of an actor is similar to the relation of transfer objects, except for the absence of the `required` and `default` modifiers; the remaining modifiers are identical to those of the transfer object relation. The full list of modifiers can be found in the xref:appx_modifiers.adoc#actor-access[Appendix].
