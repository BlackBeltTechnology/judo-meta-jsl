...
The fetching strategy determines the timing of fetching entity member values and related entities from persistent storage. There are two ways available for fetching values and related entities.

// Lazy? Eager?

* *Embedded fetching* is a technique where values are fetched at the same time as the owner entity is retrieved from persistent storage. Its name refers to the fact that the value is embedded into the owner entity. This approach can be beneficial when dealing with simple getter expressions and only a few related objects, as it helps eliminate the need for additional fetches and improves the application's performance.

* *Requested fetching* is a technique where values are *not* fetched at the same time as the owner entity is retrieved from persistent storage. Instead, they are retrieved only when requested for the first time. Its name refers to the fact that the value is calculated and retrieved upon request. This approach ensures that values are loaded from persistent storage only as they are needed.

////
* *Query fetching* is a technique where values are *not* fetched at the same time as the owner entity is retrieved from persistent storage. Instead, they are retrieved only when accessed for the first time. Its name refers to the fact that the value is calculated and retrieved in a separate database query. This approach ensures that values are loaded from persistent storage only as they are needed.
////

By default, the following fetching strategies are applied to entity members:

* compositions and fields with no parameters use embedded fetching

* relations use requested fetching

* calculated members that have parameters use requested fetching

=== Fetching annotations

The default fetching strategy of the entity members can be altered by applying annotations. It is important to note that applying fetching annotations does not modify the business logic, but significantly impacts the performance of the application.

To apply the requested fetching strategy on an entity member, use the `@Requested` annotation.
...
In the above example, the `price` field will not be calculated and loaded when the order entity is fetched. Instead, it will be calculated only when a read request is made for the price of the order.

To apply the embedded fetching strategy on an entity member, use the `@Embedded` annotation.
...
In the above example, the `orders` relation will be loaded when the order entity is fetched. 
