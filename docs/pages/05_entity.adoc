...
where the <entity> is the name of an entity, and <name> is the referable name of the calculated member. The optional `[]` indicates that the calculated member returns a collection rather than a single reference to one <entity>.

The getter <expression> returns the value of the derived member whenever it is evaluated. The moment of evaluation needs explanation. Calculated fields that return a primitive value are evaluated immediately when the owner entity is referenced. However, the getter <expression> of calculated relations are only evaluated during navigation.
...
The optional comma separated list is the arguments of the <expression>. The arguments can be used in the <expression>. <primitive> is the type of argument which can be a domain model primitive only. <argument> is the name of the argument.

Arguments can be declared with optional <default> values. The default values are used when the query is executed by one or more missing arguments. The default value is an expression, but `self` cannot be used in default expressions.
