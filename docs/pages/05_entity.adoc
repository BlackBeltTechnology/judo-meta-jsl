...
In the above example, the `price` field will not be calculated and loaded when the order entity is fetched. Instead, it will be calculated only when a read request is made for the price of the order. Please note that for non-calculated primitive fields, only eager fetching is permitted.
