...
where the <basetype> is one of the base types defined above, the <name> is the name of the domain model primitive, and there can be an additional list of modifiers for each base type. See more about modifiers in xref:02_syntax.adoc#modifiers[Modifiers] chapter.

Modifiers are constraints that restrict the use of the newly created primitive data type.
...
* `shop::OrderStatus#OPEN == shop::OrderStatus#ORDERED` is `false`
