...
where the <name> is the name of the export, and the services are defined between `{` and `}`. Possible service definitions are provided in the next section.
...
Mapped exports also groups services, but they are linked to a specific entity instance, similar to mapped transfer objects. The link between the mapped export and the entity is represented by a specific field, called mapping field.
...
where the <name> is the name of the export, and the services are defined between `{` and `}`. The <entity> is the entity type to which the export is linked and <mapping-field> is the name of the field that can be referenced. Possible service definitions are provided in the next section.
...
The above example defines a mapped export named "PersonExport" with a single service member. Within the body of the service, the mapped field is available using the variable `person`. For example, the body of the deletion service can be
...
The example above defines a guard for export, and the services are not executed if that person has a lead.
...
The example above defines a guard for export, and the salesperson can only log in if he has at least one lead.

=== CRUD annotations

Simple create, delete and update services are provided automatically. We can use `@Create`, `@Delete` and `@Update` annotations to specify these services.


Use `@Create` annotation on an action service to define a new create service. The create service creates a new transfer object as defined by its return type. The new transfer object will not be persisted, but only transfer object is returned.

*Example:*

----
@Create
service InvoiceTransfer myCreate();
----

@Create annotation can be used only on services that

* have a mapped transfer object return type
* have no parameter
* have no body
...
Use `@Update` annotation on an action service to define a new update service. The update service persists the mapped transfer object passed as its argument. If the mapped entity of the transfer object does not exist, it creates a new entity. If the mapped entity already existed, it updates the entity.

*Example:*

----
@Update
service update(InvoiceTransfer invoiceTransfer);
----
