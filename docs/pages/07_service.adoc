...
where <field> is a field of the transfer object having the constructor and <expression> is the default value of the field. The expression cannot contain reference to the mapping field.
...
transfer LeadTransfer(Lead lead) {
    field Integer value reads lead.value;
}
...
[judo-req="true",judo-req-id="REQ-SRV-009"]
=== Function
