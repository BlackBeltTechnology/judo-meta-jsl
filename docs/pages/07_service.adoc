...
The optional <type> is the output type of the service. If there is no <type> defined or the return type is `void`, the service cannot return any data. The <type> can be one of the followings:
...
----
service <transfer>[[]] <name> => <expression> ;
----
...
----
export PersonExport(Person person) {
    service PersonTransfer getManager() => person.manager ;
}
----
...
// guard for login and/or login service
