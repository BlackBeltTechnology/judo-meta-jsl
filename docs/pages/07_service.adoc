...
The mapping is a configuration approach that copies entity field values into the fields of the transfer object in a pre-defined way, see more in <<Mapping configuration>> chapter.
...
transfer PersonTransfer(Person person) {
    field String name = "John" maps person.name ;
}
----
...
----
transfer PersonTransfer {
    field required String firstName = "John";
    field required String lastName = "Doe";
    field String midName = "" ;
}
----

The example above complements the previous PersonTransfer example by adding a default value to the `firstName`, `lastName` and `midName` fields.

// read-only fields?

=== Mapping configuration

A mapping configuration is a declarative description of how the fields of a transfer object read and/or write specific fields of the entity mapped by the transfer object. Mapping configuration can only be used for mapped transfer objects.

Use the `reads` or `maps` keywords in the field definitions to define the mapping configuration.

*Syntax:*

----
field [required] <primitive> <name> [= <default>] [(reads|maps) <expression>];
----

where <primitive> is the name of a domain model primitive, or

----
field [required] <transfer>[[]] <name> [= <default>]  [(reads|maps) <expression>];
----

If the `reads` keyword is used, the value of the transfer field will be set to the value of the expression specified after `reads`. 
...
transfer SalesPersonTransfer(SalesPerson salesperson) {
    field String fullName reads salesperson.firstName + " " + salesPerson.lastName ;
}
----
...
The `reads` expressions are used whenever a transfer object or a collection of transfer objects are requested in a function, see <<Data function>> section.

In the second example, we use the `maps` keyword, which in addition to setting the value of the transfer field, writes the value of the transfer field to the entity field when the transfer object is saved.
...
In other words, `reads` creates a one-way connection between the entity and the transfer fields, where the value of the transfer field takes the value of the expression. And `maps' defines a two-way connection.

[NOTE]
====
While `reads` can be followed by any expression, if the type of its return value is the same as the type of the transfer field, the expression behind `maps' can only designate one field of the mapped entity.
====
...
transfer SalesPersonTransfer(SalesPerson salesperson) {
    field String fullName reads salesperson.firstName + " " + salesPerson.lastName ;
    field LeadTransfer[] leads maps salesperson.leads ;
}
----
...
The `maps` expressions are used whenever a transfer object is persisted, see <<CRUD annotations>> chapter.
...
The function is the piece of code (currently Java implementation) that is executed when the function is invoked.
...
where the <name> is the name of the function.
...
The body of functions cannot currently be defined in JSL. If necessary, the functions must be implemented in Java.
////
...
----
function <transfer>[[]] <name> => <expression>;
----
...
Use `@Insert` annotation on a data function to define an additional insert function in the service.

*Syntax:*

----
@Insert
function <transfer>[[]] <name> => <expression> choices <choices>;
----

The `@Insert` can only be specified for data functions where the function <expression> points to a relation of the mapped entity (see xref:06_entity.adoc[Relations]) and <choices> is specified. The insert function inserts a new member to the relation of the mapped entity. The possible new members of the relation are limited to the available <choices>. The <choices> is an expression of the same type as the collection returned by <expression>.

==== @Remove

Use `@Remove` annotation on a data function to define an additional remove function in the service.

The `@Remove` can only be specified for data functions where the function <expression> points to a relation of the mapped entity, see xref:06_entity.adoc[Relations]. The remove function removes a member from the relation of the mapped entity.
...
Use `@New` annotation on a function to define a function that creates a new transfer object. The function creates the new transfer object as defined by its return type. The new transfer object will not be persisted, only the transfer object is returned. The fields of the newly created transfer object will be set according to the default values of the fields.
