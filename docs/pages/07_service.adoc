...
The main building blocks of the service model are transfer objects, services and actors. The following chapters describe these building blocks in detail.
...
The above example declares the transfer object `PersonTransfer` with no fields.
...
----
service <name> (<entity> <mapping-field>) {
    [function] ...
}
----
...
----
service <name> maps <entity> as <mapping-field> [{
    [function] ...
}]
----
...
The function is the piece of code (JSL script or Java implementation) that is executed when the function is invoked.

The function has a name, an optional input parameter and an optional return value. The return value is a computed value passed to the function caller. This value is often referred to as output.
...
The function parameter is a variable used to configure the function. It is passed to the function in order to determine how the function will behave when it is invoked. Function parameter can be used to customize the behavior of the function for different situations.

Use the `function` keyword to specify a function.

*Syntax:*

----
function [<type>|void] <name> ([<parameter>]) [throws <error1> [, <error2>] ...] {
    <script>
}
----

where the <name> is the name of the function, and the <script> between `{` and `}` is the code to execute.

The optional <type> is the output type of the function. If there is no <type> defined or the return type is `void`, the function cannot return any data. The <type> can be one of the following:
...
The optional <parameter> is specified after the function name, inside the parentheses. It consists of a type and a name. The type is the name of a transfer object, and the parameter name can be referenced within the body of the function.

Both the return value and the parameter are required when calling the function if specified in the function declaration.

The `throws` keyword is used to declare a list of errors that may occur during the execution of the function code. This optional error list contains the names of the errors that the function may throw and the caller must be prepared to handle them.

The body of the function is optional. If no function body is specified (e.g. missing block with curly braces), the function must be implemented in Java.
...
service PersonService(Person person) {
    function void delete() ;
    function LeadTransfer[] addLead(LeadTransfer lead) ;
}
----

The above example shows two functions. The `delete` function has no return value or parameters and will be implemented in Java. The second function (`addLead`) has both a return value and a parameter. This must also be implemented in Java.

=== Data function

Data functions are used to query the data model quickly and elegantly. The data function is an expression (see Expression) that will be evaluated when the function is invoked. Data functions are very similar to queries (see Query), except that they have no input parameters and are used by external systems or users.

A very important feature of data functions is that their results can be filtered and paging is provided out of the box. Filtering can be performed on the values of the return value fields. The aim of the paging is, dividing the result into discrete pages with the help of the function.
...
----
function <primitive> <name> => <expression> ;
----
...
----
function <transfer>[[]] <name> => <expression> [options <options>];
----
...
[NOTE]
====
The <options> can only be specified for data functions where the function <expression> specifies a direct relationship of the mapping field.
====
...
The example above returns a PersonTransfer object that is the manager of the person represented by the mapping field. The expression selects a Person entity, but the return value of the service is a PersonTransfer mapped transfer object. The mapping between the result of the expression and the return value of the function is done automatically.

[NOTE]
====
A data function expression always works with entities and their relationships, but the return value of a data function is always a mapped transfer object. The mapping between the two is always done according to the mapping rules defined for the mapped transfer object (see default expression).
====
...
The above example defines an actor named `SalesPersonActor`. The company's sales staff are managed in the "COMPANY" realm. The salespersons' unique identifier is their email address.
...
----
actor <name> ([<entity> <mapping-field>]) [exports <service>, ...]
    [realm "<realm>"]
    [identity <identity>];
----
...
The example above allows the manager to create a new person. To grant permissions to external users, we first need to define entities, transport objects and services. The Manager actor in this application has no realm or identity, meaning this actor is an anonymous actor. Anonymous actors are publicly available, anyone who knows the URL of the Manager actor interface can invoke the create function. This way, the Manager actor is given access to the ManagerExport's create function, with which the user can create a PersonTransfer.

*Permission graph*

// https://en.wikipedia.org/wiki/Graph-based_access_control

JUDO's security mechanism is based on a logical permission graph, where the entry point for external users to the application is the actor. Calling the functions exported by the actor, the user can access transfer objects. Once the user has access to a transfer object, they can call the services exported by the transfer object (and can access further transfer objects).

The primary benefit of this mechanism is that it provides a direct mapping between the user's permissions and the transfer objects and functions they can access. This makes it easier to manage permissions and control access to the application.
...
----
transfer <name> (<entity> <alias>) [exports <service>, ...] [{
    [member] ...
}]
----
...
The complete authorization mechanism can be defined by carefully provisioning services to actors and transfer objects. Always be aware of that if a transfer object has already been received by an external user, then they are authorized to call the functions provided by the transfer object.
...
To add a guard to functions use `guard` as follows:
...
The example above defines a guard for service, and the functions are not executed and run-time error is thrown if that person has a lead.
...
Simple new, create, delete and update functions are provided automatically. We can use `@New`, `@Create`, `@Delete` and `@Update` annotations respectively to specify these functions.


==== @New

Use `@New` annotation on a function to define a function that creates a new transfer object. The function creates the new transfer object as defined by its return type. The new transfer object will not be persisted, only the transfer object is returned.
...
@New annotation can be used only on functions that
...
The above example declares the myCreate function, which creates a new Invoice entity and sets only the entity's customerName field. The comment field is not persisted because the comment field of the transfer object is not marked with `@Mapped`.
...
The above example declares the myUpdate function, which sets only the entity's customerName field. The comment field is not persisted because the comment field of the transfer object is not marked with `@Mapped`.
...
Use `@Delete` annotation on a function to define a new delete function. The delete function deletes the entity referenced by its export's mapping field.
