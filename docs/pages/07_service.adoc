...
Unmapped exports are simple components that group only static services. See static services later.
...
The example above defines an export named `ManagerExport` with only one static service member.

=== Mapped export

Mapped exports also groups services, but they are linked to a specific entity instance. Mapped export can contain static and non-static services 

Mapped transfer objects also carry data between our application and the outside world, but they can be linked to a specific entity instance. The link between the mapped transfer object and the entity is represented by a specific field of the mapped transfer object, called mapping field.
