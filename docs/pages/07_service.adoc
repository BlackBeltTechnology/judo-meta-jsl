...
[NOTE]
====
While `reads` can be followed by any expression, if the type of its return value is the same as the type of the transfer field, the expression behind `maps' can only points to one field of the mapped entity.
====
...
The `@Insert` can only be specified for data functions where the function <expression> points to a relation of the mapped entity (see xref:06_entity.adoc[Relations]) and <choices> is specified. Moreover, the function must return a collection.
